...
Write-Host "  GPS active time  : should be < 5 % of the $Minutes-min window (~$([math]::Round($Minutes * 0.05 * 60)) s)"
Write-Host "  Wakelock duration: foreground service wakelock is expected; others flag issues"
Write-Host "  Estimated power  : < 5 mAh per hour is typical for this type of service"
Write-Host ""
Write-Host "Red flags:" -ForegroundColor Red
Write-Host "  GPS on for > 20 % of the time  → stream accuracy may be too high (check LocationConfig)"
...
Write-Host ""
$dumpFile = "battery_dump_$(Get-Date -Format 'yyyyMMdd_HHmmss').txt"
